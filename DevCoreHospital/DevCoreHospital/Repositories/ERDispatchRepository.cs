...
namespace DevCoreHospital.Repositories
{
    ...
    {
        private readonly IERDispatchDataSource dataSource;
        ...
        public IReadOnlyList<DoctorRosterEntry> GetDoctorRoster()
        {
            ...
            return dataSource.GetRosterEntries()
                .Where(IsDoctor)
        ...
        public int CreateIncomingRequest(string specialization, string location)
            => dataSource.CreateRequest(specialization, location, "PENDING");

        public ERRequest? GetRequestById(int requestId) => dataSource.GetRequestById(requestId);

        public DoctorRosterEntry? GetDoctorById(int doctorId)
        {
            var now = DateTime.Now;

            return dataSource.GetRosterEntriesByStaffId(doctorId)
                .Where(IsDoctor)
                .Where(entry => IsOnCurrentShift(entry, now))
                .Select(NormalizeRosterEntry)
        ...
        public void UpdateRequestStatus(int requestId, string status, int? doctorId, string? doctorName)
            => dataSource.UpdateRequestStatus(requestId, status, doctorId, doctorName);

        public void UpdateDoctorStatus(int doctorId, DoctorStatus status)
            => dataSource.UpdateDoctorStatus(doctorId, status);
